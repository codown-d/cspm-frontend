...
export type TzCustomFilterRangePickerProps = Omit<
  TzDatePickerProps,
  'value'
> & {
  value?: [moment.Moment, moment.Moment] | null | undefined;
  onChange?: (
    arg: [moment.Moment?, moment.Moment?] | undefined,
    arg2?: [string?, string?],
  ) => void;
  className?: string;
  showTime?: boolean;
...
const TzCustomFilterRangePicker = (props: TzCustomFilterRangePickerProps) => {
  ...
  useEffect(() => {
    ...
    setEndWid(
      startValIsNull ? originWid.current[1] + ICON_WID : originWid.current[1],
    );
  ...
  const handleBlur = useCallback(
    (name: 'start' | 'end', isFocus: boolean = false) => {
  ...
  return (
    <>
      <div className={mergeClassName}>
        <TzDatePicker
          disabledDate={
            endTime ? (current) => current && current > endTime : undefined
          }
          {...startMergeProps}
          placeholder={intl.formatMessage({
            id: endMergeProps.showTime
          ...
          style={{
            ...
            width:
              focusedDate.start || startTime ? startWid || 'calc(50%)' : 70,
          }}
          value={startTime}
        ...
        <TzDatePicker
          disabledDate={
            startTime ? (current) => current && current < startTime : undefined
          }
          {...endMergeProps}
          placeholder={intl.formatMessage({
            id: endMergeProps.showTime
