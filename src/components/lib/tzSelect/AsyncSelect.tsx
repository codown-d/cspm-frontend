...
const AsyncSelect = forwardRef((props: TzAsyncSelect, ref?: any) => {
  ...
  const [searchValue, setSearchValue] = usePropsAttr<string | undefined>(
    props,
    'searchValue',
  );
  ...
  const [selectedList, setSelectedList] = useState<
    TzSelectCommonProps['options']
  >([]);
  const [valueChangedNum, setValueChangedNum] = useState<number>(0);
  const offsetRef = useRef<number>(0);
  const openRef = useRef<boolean>(!!open);

  const _value: string[] | undefined = isArray(value)
    ? value
    : isNil(value)
    ? value
    : [value];

  const setSateFn = useMemoizedFn(
    (setSate: Dispatch<SetStateAction<any>>, val) => {
  ...
  const { data, loading, loadMore, loadingMore, reload } = useInfiniteScroll(
        ...
        .pipe(
          map((res) => {
            const resList = (get(res, 'list') || []).filter(
              (v: any) =>
                !firVals.current?.includes(v[fieldNames?.value ?? 'value']),
            );
            offsetRef.current += 1;
        ...
        .toPromise(),
    {
      isNoMore: (d) =>
        isTokenType
          ? d?.nextId === undefined
          : offsetRef.current * PAGESIZE > (d?.total || 0),
    },
  );
  ...
  const mregeSelectedList = useMemo((): TzSelectCommonProps['options'] => {
    ...
    const filterList = searchValue
      ? selectedList?.filter((v) =>
          toUpper(v[fieldNames?.label ?? 'label']).includes(
            toUpper(searchValue),
          ),
        )
      : selectedList;
      ...
      notFoundContent={
        loading || searchValue !== debouncedSearchValue ? (
          <Empty
            ...
            image={
              <LoadingOutlined
                style={{ marginTop: 10, fontSize: 24, color: '#2177D1' }}
              />
            }
          />
        ) : (
