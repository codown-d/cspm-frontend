...
import TzCustomFilterRangePicker, {
  TzCustomFilterRangePickerProps,
} from './component/TzCustomFilterRangePicker';
import TzFilterCascader, {
  TzFilterCascaderProps,
} from './component/TzFilterCascader';
import TzFilterInput, { TTzFilterInput } from './component/TzFilterInput';
import TzFilterSelect, {
  TzFilterSelectProps,
} from './component/TzFilterSelect';
import {
...
const RenderItem = (props: RenderItemProps) => {
  ...
  const mergePropsWithRangePickerCt = (
    defaultProps: FilterCustomRangePicker['props'],
  ) => {
    defaultProps &&
      wids?.[name] &&
      set(defaultProps, ['style', 'width'], wids?.[name]);
    return {
      ...defaultProps,
      bordered: false,
      inputReadOnly: true,
      popupClassName: classNames(
        'tz-filter-picker-panel',
        defaultProps?.popupClassName,
      ),
  ...
  const renderItem = useMemo(() => {
    ...
    switch (type) {
      ...
      case 'select': {
        return (
          <TzFilterSelect
            ...
            {...(mergeProps as TzFilterSelectProps)}
          />
        );
      }
      case 'cascader': {
      ...
      case 'timePicker': {
        return <TimePicker {...(mergeProps as TimePickerProps)} />;
      }
      case 'rangePicker': {
      ...
      case 'rangePickerCt': {
        return isFilter ? (
          ...
          <TzCustomFilterRangePicker
            {...(mergeProps as TzCustomFilterRangePickerProps)}
          />
        );
      }

      default:
        console.error('The component type is undefined !');
        return <></>;
    }
  }, [type, getMergeProps]);
