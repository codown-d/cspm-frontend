...
const useTzCascaderPanel = (
  param: TTzCascaderPanel,
): TTzCascaderPanelRetrun => {
  const {
    options,
    value,
    onChange,
  ...
  const [queryLoadingId, setQueryLoadingId] = useState<
    ValueType[] | undefined
  >();
  ...
  const [optionsLoadData, setOptionsLoadData] = useLocalStorageState<
    any | undefined
  >(`options-load-data-${cacheUrl}`, undefined);
  ...
  const optionDataMap = useCreation(() => {
    ...
    const needSort =
      (options?.length || 0) > (tempOptionsRef.current?.length || 0);
    ...
    return {
      ...
      lastSectedVal:
        sortOptionsData?.lastSectedVal || menuRenderRef.current?.nodeVal,
    };
  }, [options, open]);

  const [lastSectedVal, setLastSectedVal] = useState<SingleValueType>();

  const { options: optionData, lastSectedVal: optionDataMapLastSectedVal } =
    optionDataMap;
  ...
  const optLastSectedVal = useMemo(() => {
    ...
    return findNodeByValue(
      optionDataMapLastSectedVal as ValueType | undefined,
      data,
    );
  }, [optionDataMapLastSectedVal, data]);

  useEffect(() => {
    setLastSectedVal((prev) =>
      !isEqual(prev, optLastSectedVal) ? optLastSectedVal : prev,
    );
  }, [optLastSectedVal]);
  ...
  useUpdateEffect(() => {
    open &&
      setPanelVal((prev) =>
        !isEqual(orderBy(dataVal), orderBy(prev)) ? dataVal : prev,
      );
  ...
  const trigger = useMemoizedFn((val) => {
    ...
    if (!multiple) {
      onChange(opts?.[0]?.map((item) => item.value), opts?.[0]);
    } else {
      onChange(opts?.map((item) => item?.map((v) => v.value)), opts);
    }
  });
