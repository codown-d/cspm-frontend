...
import { assign, get, isArray, isEqual, keys, set } from 'lodash';
...
const reducer = produce((draft: TzFilterState, action: FilterDataAction) => {
  ...
  switch (action.type) {
    case UPDATE_FILTER: {
      ...
      !isEqual(_payload.filterFormItems, get(draft, 'filterFormItems')) &&
        set(draft, 'filterFormItems', _payload.filterFormItems);
      !isEqual(_payload.enumLabels, get(draft, 'enumLabels')) &&
        set(draft, 'enumLabels', _payload.enumLabels);
      !isEqual(_payload.fitlerFormValues, get(draft, 'fitlerFormValues')) &&
