...
export const getFormItemByItems = (
  params: TzFilterParams | FilterFormParam[],
  setConditinDefaultVal?: boolean,
): TzFilterState => {
  let items;
  ...
  const filterFormItems = items
    // .filter(
    //   (v) =>
    //     (valueKeys.includes(v.name) || hasIn(v, 'value')) &&
    //     !valueIsNull(v.value || initialValues?.[v.name]),
    // )
    .map((v: FilterFormParam) => {
      const { value, ...rest } = v;
      ...
      if (v.type === SELECTTYPE && v.condition) {
        ...
        setConditinDefaultVal &&
          (val = {
            ...val,
            [v.condition.name]:
              initialValues?.[restCon.name] ??
              get(restCon, ['props', 'options', 0, 'value']),
          });
        return {
          ...rest,
          condition: {
...
export const defaultData: FilterData = {
  inputFilterData: [],
  popoverFilterData: [],
};
