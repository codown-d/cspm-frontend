...
type RadarProps = {
  loading?: boolean;
  title?: string;
  dataSource?: API_AGENTLESS.VulnRiskInfoResponseAttr[];
};
function Radar({ dataSource, title, loading: loadingProps }: RadarProps) {
  ...
  const transData = useMemoizedFn(
    (data: API_AGENTLESS.VulnRiskInfoResponseAttr[]) => {
      if (!data?.length) {
        setIsEmpty(true);
        return {};
      }
