...
const fieldName2Val = (
  data: TzSelectBaseProps['options'],
...
const TzSelectCommon = forwardRef((props: TzSelectCommonProps, ref?: any) => {
  ...
  const triggerSearchValue = (val?: string) =>
    !hasIn(props, 'searchValue') && setSearchValue(val);

  const triggerOpen = (val?: boolean) => !hasIn(props, 'open') && setOpen(val);

  const showCheckbox =
    mode && ['multiple', 'tags'].includes(mode) && itemRenderWithCheckbox;
  ...
  return (
    <TzSelectBase
      {...(realProps as TzSelectBaseProps)}
      value={value}
      ref={ref}
    />
  );
